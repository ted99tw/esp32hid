...
static void read_joystick_task(void *pvParameter)
{
    ...
    while(true) {
        ...
        if (xQueueReceive(joystick_buttons_events, &ev, 1/portTICK_PERIOD_MS)) {
        ...
        //current_sum = js2x + js2y + js1x + js1y;

        ESP_LOGD(HID_JOYSTICK_TAG, "last_sum %d", last_sum);

        if (abs(js1x - 127) + abs(js1y - 127) > 5){
            ESP_LOGI(HID_JOYSTICK_TAG, "send buttons %d JS1 X=%d Y=%d JS2 X=%d Y=%d", buttons, js1x, js1y, js2x, js2y);
...
/*
static esp_err_t read_mpu6050_init(void)
{

    xTaskCreate(read_mpu6050_task, "read_mpu6050_task", 2048, NULL, 4, NULL);
